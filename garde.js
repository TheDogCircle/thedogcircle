...
(function () {
  ...
  function injectGardeUI() {
    ...
    subtabs.innerHTML =
      ...
      + '<button class="subtab" id="gTabDemandes">🐾 Faire une demande</button>'
      + '<button class="subtab" id="gTabProposer">➕ Proposer ma dispo</button>';

    sec.insertBefore(subtabs, document.getElementById('gardeGrid'));

    // Panneau demande
    var panelDemande = document.createElement('div');
    panelDemande.id    = 'garde-demande';
    panelDemande.style = 'display:none;';
    panelDemande.innerHTML =
      '<div style="background:var(--w);border:1px solid var(--b);border-radius:16px;padding:24px;max-width:500px;">'
        ...
        + '<div style="margin-bottom:20px;">'
          + '<label style="display:block;font-size:11px;color:var(--t3);text-transform:uppercase;margin-bottom:5px;">Message (infos utiles)</label>'
        ...
        + '<button id="dem-submit" class="btn btn-p" style="width:100%;padding:12px;font-size:14px;">Envoyer ma demande 🐾</button>'
        + '<div id="dem-ok" style="display:none;background:#d1fae5;color:#065f46;border-radius:10px;padding:12px;text-align:center;font-size:13px;margin-top:12px;">✓ Demande envoyée ! Un membre vous contactera 🐾</div>'
      + '</div>';
    sec.appendChild(panelDemande);
      ...
      '<div style="background:var(--w);border:1px solid var(--b);border-radius:16px;padding:24px;max-width:500px;">'
        ...
        + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">'
          + '<div><label style="display:block;font-size:11px;color:var(--t3);text-transform:uppercase;margin-bottom:5px;">Ton prénom</label>'
            + '<input type="text" id="pro-prenom" placeholder="Sophie" style="width:100%;padding:10px 12px;border-radius:10px;border:1.5px solid var(--b);font-family:inherit;font-size:14px;background:var(--cream);color:var(--t);outline:none;" value="' + (window.TDC.userPrenom || '') + '"></div>'
        ...
        + '</div>'
        + '<div style="margin-bottom:20px;">'
          + '<label style="display:block;font-size:11px;color:var(--t3);text-transform:uppercase;margin-bottom:5px;">Préférences (gabarits, nb chiens...)</label>'
        ...
        + '<div id="pro-ok" style="display:none;background:#d1fae5;color:#065f46;border-radius:10px;padding:12px;text-align:center;font-size:13px;margin-top:12px;">✓ Disponibilité publiée ! Elle apparaît dans la liste 🐾</div>'
      + '</div>';
    sec.appendChild(panelProposer);

    // Bind sous-onglets
    document.getElementById('gTabGardiens').addEventListener('click', function () { showGardeTab('gardiens'); });
    document.getElementById('gTabDemandes').addEventListener('click', function () { showGardeTab('demande'); });
  ...
  function showGardeTab(tab) {
    ...
    document.getElementById('gTabDemandes').classList.toggle('active', tab === 'demande');
    ...
    document.getElementById('garde-demande').style.display  = tab === 'demande'  ? 'block' : 'none';
    document.getElementById('garde-proposer').style.display = tab === 'proposer' ? 'block' : 'none';
  }

  // ── Charger les gardiens ─────────────────────────────
  function loadGarde() {
    ...
    window.TDC.db
      ...
      .then(function (res) {
        ...
        if (!res.data || res.data.length === 0) {
          g.innerHTML = '<div class="loading" style="grid-column:1/-1;">Aucun gardien disponible pour le moment 🐾<br><small style="color:var(--t3);">Propose ta disponibilité !</small></div>';
          return;
        }
        ...
        g.innerHTML = res.data.map(function (gd, i) {
          ...
          return '<div class="garde-card">'
            ...
            + '<button class="btn btn-p" style="padding:6px 14px;font-size:12px;width:100%;justify-content:center;" onclick="window.location.href=\'mailto:thedogcircleclub@gmail.com?subject=Garde - ' + encodeURIComponent(gd.prenom) + '\'">Contacter</button>'
          + '</div>';
        }).join('');
      });
  }
  ...
  async function saveDemande() {
    ...
    if (res.error) { err.textContent = 'Erreur : ' + res.error.message; err.style.display = 'block'; }
      ...
      setTimeout(function () { document.getElementById('dem-ok').style.display = 'none'; }, 4000);
    }
  ...
  async function saveProposition() {
    ...
    if (res.error) { err.textContent = 'Erreur : ' + res.error.message; err.style.display = 'block'; }
      ...
      setTimeout(function () { document.getElementById('pro-ok').style.display = 'none'; }, 4000);
      loadGarde();
    }
